import type { CompletionReport, ConsolidatedReport, DashboardAggregate } from '../api/pulse';

/** Turns a team dashboard aggregate into a handful of plain-language bullets for the executive
 * summary at the top of the weekly/team report and historical snapshot pages. */
...
export function summarizeConsolidated(report: ConsolidatedReport): string[] {
  ...
  if (criticalAlerts.length > 0) {
    points.push(`${criticalAlerts.length} critical alert(s) across missions, most urgently from ${criticalAlerts[0].missionName}: "${criticalAlerts[0].content}".`);
  }
